...
test_copy_and_compare()
{
  ...
  ok(ulonglong2decimal(val,&d1) == 0, "Pass1");
  d2= d1;
  my_decimal d3(d1);

  ok(my_decimal_cmp(&d1, &d2) == 0, "Pass2");
  ok(my_decimal_cmp(&d2, &d3) == 0, "Pass3");
  ok(my_decimal_cmp(&d3, &d1) == 0,"Pass4");

  ulonglong val1, val2, val3;
  ok(decimal2ulonglong(&d1, &val1) == 0, "Pass5");
  ok(decimal2ulonglong(&d2, &val2) == 0,"Pass6");
  ok(decimal2ulonglong(&d3, &val3) == 0,"Pass7");
  ...
  val= 45;
  ok(ulonglong2decimal(val, &d1) == 0,"Pass11");
  ok(my_decimal_cmp(&d1, &d2) == 1,"Pass12");
  ok(my_decimal_cmp(&d1, &d3) == 1,"Pass13");
