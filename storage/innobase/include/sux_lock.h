...
public:
  ...
  bool not_recursive() const { return UNIV_LIKELY(!recursive); }

  /** Acquire a recursive lock */
  template<bool allow_readers> void writer_recurse()
  {
    ut_ad(writer == os_thread_get_curr_id());
    ut_ad(recursive / (allow_readers ? RECURSIVE_U : RECURSIVE_X) <
          RECURSIVE_MAX);
...
private:
  ...
  void new_owner(os_thread_id_t id)
  {
  ...
  void first_owner(os_thread_id_t id)
  {
    IF_DBUG(DBUG_ASSERT(!writer.exchange(id)), writer= id);
  }
public:
  ...
  void x_unlock() { if (!writer_unlock(true)) read_lock.wr_unlock(); }
